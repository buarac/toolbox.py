...
def setup_args():
    ...
    parser.add_argument(
        "--output", "-o", type=Path, help="Output directory (default: input/resized)"
    )
    ...
    parser.add_argument(
        "--quality", "-q", type=int, default=85, help="JPEG Quality (1-100)"
    )
    return parser.parse_args()


def process_image(
    file_path: Path, output_dir: Path, width: int, height: int, quality: int
):
    try:
        with Image.open(file_path) as img:
            ...
            logging.info(
                f"✅ Processed: {file_path.name} -> {output_file} (Quality: {quality})"
            )
